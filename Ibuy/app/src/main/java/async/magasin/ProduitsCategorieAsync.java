...
public class ProduitsCategorieAsync extends AsyncTask<Integer, Void, List<ProduitView>> {
    private Activity activity;

    @Override
    protected List<ProduitView> doInBackground(Integer... params) {
        try {
            String url = WSUtil.getUrlServer() + "/magasins/" + params[0] + "/categories/" + params[1];
    ...
    protected void onPostExecute(List<ProduitView> produits) {
        if (produits != null) {
            final RecyclerView recyclerView = (RecyclerView) activity.findViewById(R.id.recListProduit);
